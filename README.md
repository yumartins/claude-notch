...
macOS **menu bar** app that shows, at a glance, which **Claude Code** sessions are
stalled waiting for you (permission prompt or idle). Built with **Tauri v2** and a
**React** frontend (Vite + Tailwind + shadcn/ui on Base UI).
...
- **Session overview**: every Claude Code session with live activity (current tool,
  e.g. `Bash · cargo build`), status dot (amber = waiting, green = running) and age.
- **Expandable details**: last assistant message, the action asking for permission,
  full path, terminal + tty, context/output token usage.
- **Approve / deny from the popover**: focuses the session's terminal and sends the
  keystroke Claude Code expects (`1` / Esc). Needs the Accessibility permission.
...
1. Claude Code hooks (`Notification`, `Stop`, `UserPromptSubmit`) write each session's
   state to `~/.claude/status/<session_id>.json` — including `tty` and `TERM_PROGRAM`
   for terminal focusing.
2. The Rust backend polls that folder every ~1.5s, updates the menu bar icon
   (amber + count) and pushes the session list to the popover.
3. Clicking the icon toggles the popover, positioned right under it
...
scripts/             Claude Code hook (status.py) + example settings
```
...
cp scripts/status.py ~/.claude/hooks/status.py
chmod +x ~/.claude/hooks/status.py
```

Then copy the contents of `scripts/claude-settings.example.json` into
`~/.claude/settings.json` (merging with whatever you already have under `hooks`).
